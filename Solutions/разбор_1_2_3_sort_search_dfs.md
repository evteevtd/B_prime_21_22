...
- Важным условием окажется то, что из вершины i ведут есть ребро какого-то цвета в i + 1.
- Пусть dfs1 ходит по ребрам цвета 1, dfs2 - цвета 2. Массивы used(1, 2) отвечают за то, была ли вершина посещена в dfs. 
- Запустим dfs1 из вершины 1, если придем в помеченную used2 вершину - ответ NO. 
- Запустим dfs2 из вершины 1, если придем в помеченную used1 вершину - ответ NO. 
- Не обнуляя массивы used проделаем то де самое из вершины 2.
- Решение могло бы не сработать если на какой-то итерации мы бы нашли вершину, покрашенную на предыдущих итерациях, но в данной задаче это не проблема, почему? 
- потому что на итерации i хотя бы в одном из графов (цвета 1 или 2) вершина i уже была посещена ранее (так как есть ребро i-1 -> i какого-то цвета). А значит мы запустим dfs только в одном графе, и будем смотреть только в корректный used другого цвета.
